export default function DashboardFooter({ darkMode, onShowTerms }) {
  return (
    <footer className={`${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-100 border-gray-200'} text-center p-4 text-sm border-t transition-opacity duration-300`}>
      ...
      <p className="mt-2">
        <button 
          onClick={onShowTerms}
          className={`${darkMode ? 'text-emerald-400 hover:text-emerald-300' : 'text-emerald-600 hover:text-emerald-500'} transition-colors duration-300 underline`}
        >
          Privacy Policy
        </button> 
